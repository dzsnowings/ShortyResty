...
func handleRedirect(w http.ResponseWriter, r *http.Request) {
    ...
    var id string = shortUrl.Short_url[len(shortUrl.Short_url) - 8:]        // Get ID of the ShortUrl
    if _, key := urlMap[id]; key {
        http.Redirect(w, r, urlMap[id].Long_url, 302)       // 302 redirect to the long URL
    } else {
        w.Write([]byte(shortUrl.Short_url + " is not linked to a long URL."))       // Respond with error message if id is not in urlMap
        return
    }
}
